...
export const useQuizStore = defineStore('quiz', () => {
  ...
  const theme = computed<QuizThemeConfig>(() => ({
    ...
    backgroundType: config.value?.theme?.backgroundType ?? 'color',
    backgroundGradient: config.value?.theme?.backgroundGradient ?? '',
    backgroundEmoji: config.value?.theme?.backgroundEmoji ?? '✨',
